...
const FEATURES = [
  {
    image: "/assets/feature_table_delivery.png",
    title: "On Table Delivery",
    description: "Scan the QR at your desk. Your order from Noiric Cafe arrives hot and on time — no counter trips.",
    tag: "Collab",
    tagColor: "bg-accent-birch-wood/15 text-[#A67C52] border-accent-birch-wood/25",
    collab: true,
  },
  {
    image: "/assets/feature_membership_offer.png",
      ...
      <div className="w-full max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col gap-4 mb-16 max-w-2xl">
          ...
          <h2 className="shrimp-display text-4xl md:text-5xl lg:text-6xl text-ink-primary leading-[0.95]">
            THE MINDSPACE{" "}
            <span className="text-accent-aqua-core">EXPERIENCE</span>
          </h2>
          <p className="text-ink-secondary text-sm md:text-base leading-relaxed max-w-lg">
            Every detail is engineered for your focus. From cafe partnerships to
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
          {FEATURES.map((feature, idx) => (
            <div
              key={idx}
              className={`group relative bg-bg-warm-elevated rounded-2xl border overflow-hidden shadow-sm hover:shadow-lg transition-all duration-500 ease-out hover:-translate-y-1.5 ${
                feature.collab
                  ? "border-accent-birch-wood/30 hover:border-accent-birch-wood/50"
            ...
            >
              ...
              <div className="relative w-full h-48 overflow-hidden">
                ...
                {feature.collab && (
                  <div className="absolute bottom-3 left-3">
                    <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-white/90 text-[#A67C52] border border-accent-birch-wood/20 backdrop-blur-sm shadow-sm">
                      <span className="w-1.5 h-1.5 rounded-full bg-accent-birch-wood" />
                      Noiric Cafe
                    </span>
                  </div>
                )}
              ...
              <div className="p-5 flex flex-col gap-2.5">
                <h3 className="shrimp-display text-lg text-ink-primary leading-tight">
        ...
        <div className="mt-14 flex flex-col sm:flex-row items-center justify-center gap-6 text-center">
          <p className="text-sm text-ink-muted font-medium">
            Powered by Noiric Cafe × MindSpace Partnership
          </p>
          <button
            onClick={() => {
            ...
            className="px-8 py-3 bg-ink-primary hover:bg-[#2c3632] text-white text-xs uppercase tracking-widest font-bold rounded-full transition-all duration-300 active:scale-[0.97] cursor-pointer shadow-sm"
          >
            Reserve Your Desk →
          </button>
        </div>
